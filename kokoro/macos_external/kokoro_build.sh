...
ulimit -c unlimited && (python3.7 tests/fft_test.py || (lldb -c `ls -t /cores/* | head -n1` \
    --batch -o 'settings set frame-format "frame #${frame.index}: {$frame.sp} ${frame.pc}{ ${module.file.basename}`${function.name}{${function.pc-offset}}}{ at ${line.file.basename}:${line.number}}\n' -o 'thread backtrace all' -o 'quit' && exit 1))
